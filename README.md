...
| `repo-structure-audit` | Defines the standard repo shape and audits projects against it, including root files, agent context, settings, docs placement, and migration plans. | You are starting a repo, checking root-file sprawl, moving a project into a cleaner layout, or auditing projects under `~/Code/`. |
...
| `context-window-audit` | Audits startup context, command wrappers, skill routing, memory summaries, and settings for context-window bloat or dead routes. | You ask for a context-window audit, token optimization, settings cleanup, or why an agent session is loading too much context. |
